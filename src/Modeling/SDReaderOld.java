...
public class SDReaderOld {
	...
	public void printInSequence(FragmentOld fragment, int indent) {
		...
		for (Node n : fragment.getNodes()) {
			for (int i = 0; i < indent; i++)
				System.out.print("\t");
			if (n.getClass().equals(FragmentOld.class)) {
				System.out.println(((FragmentOld) n).getName());
				// n.print();
				printInSequence((FragmentOld) n, (indent + 1));
	...
	public SDReaderOld(File xmlFile, int index) {
		this.index = index;
		try {
	...
	public void traceDiagram() throws UnsupportedFragmentTypeException, InvalidTagException {
		...
		if (n.getAttributes().getNamedItem("name") != null) {
			this.sd = new FragmentOld(n.getAttributes().getNamedItem("xmi:id").getTextContent(), n
					.getAttributes().getNamedItem("name").getTextContent());
		} else {
			this.sd = new FragmentOld(n.getAttributes().getNamedItem("xmi:id").getTextContent(), null);
		}
			...
			if (node.getNodeName().equals("fragment")) {
				if (node.getAttributes().getNamedItem("xmi:type").getTextContent()
					...
					if (node.getAttributes().getNamedItem("name") != null) {
						fragment = new FragmentOld(node.getAttributes().getNamedItem("xmi:id")
	...
	public void traceFragment(FragmentOld fragment, org.w3c.dom.Node node)
				...
				for (int k = 0; k < fragNodes.getLength(); k++) {
					if (fragNodes.item(k).getNodeName().equals("fragment")) {
						if (fragNodes.item(k).getAttributes().getNamedItem("xmi:type")
							...
							FragmentOld newFragment;
							if (fragNodes.item(k).getAttributes().getNamedItem("name") != null) {
								newFragment = new FragmentOld(fragNodes.item(k).getAttributes()
